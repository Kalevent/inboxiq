...
def enrich_lead_with_email(self, lead_id: str, domain: str) -> Dict[str, Any]:
    """
    Find and enrich lead email address using Hunter.io API.
    ...
    try:
        ...
        if best_match:
            ...
            # Add enrichment note
            enrichment_note = f"\n\nEmail found via Hunter.io: {best_match.get('value')}"
            ...
            if lead.notes:
                lead.notes += f"\n\nEmail found via Hunter.io: {first_email} (no name match)"
            else:
                lead.notes = f"Email found via Hunter.io: {first_email} (no name match)"
